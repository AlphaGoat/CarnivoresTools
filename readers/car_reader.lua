...
function CARReader:readTrianglesBlock (triangBlockNum)
    ...
    triBlock.Tr_Coord1 = bytes2int(Tr_CoordX1_data)

    local Tr_CoordX2_data = file:read(4)
    triBlock.Tr_Coord1 = bytes2int(Tr_CoordX2_data)

    local Tr_CoordX3_data = file:read(4)
    triBlock.Tr_Coord1 = bytes2int(Tr_CoordX3_data)

    local Tr_CoordY1_data = file:read(4)
    triBlock.Tr_Coord1 = bytes2int(Tr_CoordY1_data)

    local Tr_CoordY2_data = file:read(4)
    triBlock.Tr_Coord1 = bytes2int(Tr_CoordY2_data)

    local Tr_CoordY3_data = file:read(4)
    triBlock.Tr_Coord1 = bytes2int(Tr_CoordY3_data)
...
function CARReader:assignUVcoordsToVertices()
    ...
    for i, tri in pair(self.triangles) do
...
function CARReader:calcVertexNormals()
    ...
    for i, tri in ipairs(self.triangles) do
        ...
        if faces[point1] == nil then
            faces_to_verticecs[point1] = { {point1, point2, point3} }
        else
            faces_to_vertices[point1].insert({point1, point2, point3})
        end

        -- Assign face to point 2
        if faces[point2] == nil then
            faces_to_vertices[point2] = { {point1, point2, point3} }
        else
            faces_to_verties[point2].insert({point1, point2, point3})
        end

        -- Assign face to point 3
        if faces[point3] == nil then
            faces_to_vertices[point3] = { {point1, point2, point3} }
        else
            faces_to_vertices[point3].insert({point1, point2, point3})
        end
    end
    ...
    for j, faces in ipairs(sorted_faces_to_vertices) do
        ...
        for k, face in ipairs(faces) do
            ...
            local norm1 = L2norm(edge1[1], edge1[2])
            local norm2 = L2norm(edge2[1], edge2[2])

            local cross_vector = crossProduct(edge1, edge2)

            norm = L2norm(cross_vector[1] - cross_vector[2])

            point_normals.insert(norm)
        end
        ...
        for l, norm in ipairs(point_normals) do
            sum_vector = vectorSum(sum_vector, norm) 
        end
...
function CARReader:getElementArray()
    ...
    element_array = {}
    for tri in pair(self.triangles) do
        element_array.insert({tri.Tr_Point1, tri.Tr_Point2, tri.Tr_Point3})
    end
